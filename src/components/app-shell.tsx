  ...
  return (
    <div className="min-h-dvh bg-slate-50 text-slate-950">
      ...
      <nav data-qa-mobile-nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white px-3 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 md:hidden" aria-label="Navegação móvel">
        <Link href="/clientes" data-qa-hit-target="primary" className="mx-auto flex min-h-12 max-w-48 items-center justify-center gap-2 rounded-xl bg-slate-100 text-sm font-semibold text-slate-900"><UsersRound className="size-5" />Clientes</Link>
      </nav>
    </div>
  );
}

...
import { listContactDecisionsForOpportunitiesWithContext } from "@/services/contact-decision-service";
import { hasPermission } from "@/lib/permissions";
import { contactDecisionStatusLabels, opportunityPriorityLabels, opportunityStatusLabels } from "@/lib/labels";
import { refreshOpportunitiesAction } from "@/app/actions/opportunity-actions";
import { evaluateContactDecisionsAction } from "@/app/actions/contact-decision-actions";
...
const decisionFilters = ["UNEVALUATED", "PROCEED", "WAIT", "BLOCKED", "SUPPRESSED"] as const;
...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const decisions = await listContactDecisionsForOpportunitiesWithContext(result.context, result.items.map((item) => item.id));
  const decisionByOpportunity = new Map(decisions.map((decision) => [decision.opportunityId, decision]));
  const requestedDecision = first(params.decision);
  const decisionFilter = decisionFilters.includes(requestedDecision as (typeof decisionFilters)[number])
    ? requestedDecision as (typeof decisionFilters)[number]
    : undefined;
  const items = result.items.filter((item) => {
    if (!decisionFilter) return true;
    const decision = decisionByOpportunity.get(item.id);
    if (decisionFilter === "UNEVALUATED") return !decision;
    return decision?.status === decisionFilter;
  });
  const canManage = hasPermission(result.context.role, "opportunity:manage");
  const canEvaluate = hasPermission(result.context.role, "contact-decision:evaluate");
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        {canManage && <form action={refreshOpportunitiesAction}><button type="submit" data-qa-refresh-opportunities className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-800 sm:w-auto"><RefreshCw className="size-4" />Atualizar oportunidades</button></form>}
        {canEvaluate && <form action={evaluateContactDecisionsAction}><button type="submit" data-qa-evaluate-contact-decisions className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white sm:w-auto"><ShieldCheck className="size-4" />Avaliar contatos</button></form>}
      </div>
    </div>

    <Card className="p-4">
      <form role="search" aria-label="Filtrar oportunidades" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
        ...
        <label className="text-sm font-semibold text-slate-700">Decisão<select name="decision" defaultValue={decisionFilter ?? ""} className="mt-1 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"><option value="">Todas</option><option value="UNEVALUATED">Não avaliada</option>{Object.entries(contactDecisionStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        ...
        <div className="flex items-end gap-2 sm:col-span-2 xl:col-span-4"><button type="submit" className="min-h-11 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white">Aplicar filtros</button><Link href="/oportunidades" data-qa-hit-target="primary" className="inline-flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-slate-600 hover:bg-slate-100">Limpar</Link></div>
      </form>
    </Card>

    <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-slate-600">{items.length} {items.length === 1 ? "oportunidade" : "oportunidades"}</p><p className="hidden text-xs text-slate-400 sm:block">Ordenadas por prioridade, pontuação e momento recomendado.</p></div>

    {items.length === 0 ? <Card className="p-6 text-center"><h2 className="font-bold">Nenhuma oportunidade no momento</h2><p className="mt-2 text-sm text-slate-500">Os filtros atuais não retornaram nenhuma oportunidade real.</p>{canManage && <p className="mt-2 text-xs text-slate-400">Use “Atualizar oportunidades” para detectar novos marcos de jornada.</p>}</Card> : <div className="space-y-3">{items.map((opportunity) => <OpportunityCard key={opportunity.id} opportunity={opportunity} canManage={canManage} contactDecision={decisionByOpportunity.get(opportunity.id) ?? null} />)}</div>}
    ...
    {!canEvaluate && <p className="text-xs text-slate-500">Seu perfil pode consultar decisões de contato, sem avaliá-las.</p>}
